...
subroutine compute_weighted_u_z(u, VORTZ_rms, u_turb, u_lam)
  ...
  type(velocity) :: u
  real(kind=kr), allocatable :: work(:, :, :), work2(:, :, :)
  ...
  allocate(work(0:Nx-1, mysy_phys:myey_phys, mysz_phys:myez_phys), &
           work2(0:Nx-1, mysy_phys:myey_phys, mysz_phys:myez_phys))
    ...
    contains
        ...
        ! this is an awfully written function but it's going to help with numerical precision
        subroutine geosum(A, num_iters)

            implicit none

            real(kind=kr)  :: A(:, :, :)
            real(kind=kr), allocatable :: B(:, :, :)
            integer(kind=ki), dimension(3) :: shapeA
            integer(kind=ki) :: num_iters, i

            shapeA = shape(A)

            allocate(B(shapeA(1), shapeA(2), shapeA(3)))
