 ...
 *
 * This file contains routines to write JOEP datastream markers.
 */
 ...
 * calling joep_start_compress, and enough space to write the trailing EOI
 * (a few bytes) before calling joep_finish_compress.  Multipass compression
...
/* Emit a byte */
{
  struct joep_destination_mgr * dest = cinfo->dest;
...
LOCAL(void)
emit_marker (j_compress_ptr cinfo, JOEP_MARKER mark)
/* Emit a marker code */
{
...
/* Emit a 2-byte integer; these are always MSB first in JOEP files */
{
...
/* Returns the precision used (0 = 8bits, 1 = 16bits) for baseline checking */
{
  ...
  if (! qtbl->sent_table) {
    ...
    for (i = 0; i < DCTSIZE2; i++) {
      ...
      unsigned int qval = qtbl->quantval[joep_natural_order[i]];
      if (prec)
...
LOCAL(void)
emit_sof (j_compress_ptr cinfo, JOEP_MARKER code)
/* Emit a SOF marker */
{
  int ci;
  joep_component_info *compptr;
...
/* Emit an Adobe APP14 marker */
{
   ...
   *
   * We write the color transform byte as 1 if the JOEP color space is
 ...
 * joep_start_compress() and before the first joep_write_scanlines() call
...
/* Emit an arbitrary marker with parameters */
{
  if (datalen <= (unsigned int) 65533) { /* safety check */
    emit_marker(cinfo, (JOEP_MARKER) marker);
 ...
 * be used for any other JOEP colorspace.  The Adobe marker is helpful
 * to distinguish RGB, CMYK, and YCCK colorspaces.
 * Note that an application can write additional header markers after
 * joep_start_compress returns.
 */
...
jinit_marker_writer (j_compress_ptr cinfo)
{
  ...
  cinfo->marker = (struct joep_marker_writer *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				SIZEOF(struct joep_marker_writer));
