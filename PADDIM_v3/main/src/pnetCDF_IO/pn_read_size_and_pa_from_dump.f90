...
SUBROUTINE pn_read_size_and_pa_from_dump(error_code)
  USE defprecision_module
  USE parameter_module,   ONLY : Lmax,Mmax,Nmax,Gammax,Gammay,Gammaz,    &
  &                              B_therm,B_comp,C_Lorentz,D_visc,D_therm,D_comp,D_mag,   &
  &                              S_therm,S_comp, R, Theta! PH
  ...
  INCLUDE "mpif.h"
#endif
  INTEGER (kind=ki) :: Lmax_dump,Mmax_dump,Nmax_dump, nprocs1_dump, nprocs2_dump
  REAL(kind=kr)     :: Gammax_dump,Gammay_dump,Gammaz_dump
  REAL(kind=kr)     :: B_therm_dump,B_comp_dump, C_Lorentz_dump, &
...
!
  ...
  CALL pn_check (nfmpi_inq_varid(ncid_dump,'C_Lorentz', C_Lorentz_varid_dump)) ! PH
  ...
  CALL pn_check (nfmpi_inq_varid(ncid_dump,'D_mag' ,D_mag_varid_dump)        ) ! PH
  ...
  CALL pn_check (nfmpi_inq_varid(ncid_dump,'R' ,R_varid_dump)      )
  CALL pn_check (nfmpi_inq_varid(ncid_dump,'Theta' ,Theta_varid_dump)      )
...
#ifdef MAGNETIC
  CALL pn_check (nfmpi_inq_varid(ncid_dump,'Bx',Bx_varid_dump)      )
#ifndef TWO_DIMENSIONAL
  CALL pn_check (nfmpi_inq_varid(ncid_dump,'By',By_varid_dump)      )
#endif
  ...
  CALL pn_check( PM_NFMPI_GET_VAR_FLOAT_ALL(ncid_dump,C_Lorentz_varid_dump,C_Lorentz_dump) ) ! PH
  ...
  CALL pn_check( PM_NFMPI_GET_VAR_FLOAT_ALL(ncid_dump,D_mag_varid_dump,D_mag_dump)   ) ! PH
  ...
  IF (B_comp_dump.NE.B_comp) WRITE(*,'(a,i4,a)') "Warning: B_comp on id ",myid, &
       & " is different from value in restart file."
  IF (C_Lorentz_dump.NE.C_Lorentz) WRITE(*,'(a,i4,a)') "Warning: C_Lorentz on id ",myid, &
       & " is different from value in restart file." ! PH
  ...
  IF (D_comp_dump.NE.D_comp) WRITE(*,'(a,i4,a)') "Warning: D_comp on id ",myid, &
       & " is different from value in restart file."
  IF (D_mag_dump.NE.D_mag) WRITE(*,'(a,i4,a)') "Warning: D_mag on id ",myid, &
       & " is different from value in restart file." ! PH
  ...
  IF (S_comp_dump.NE.S_comp) WRITE(*,'(a,i4,a)') "Warning: S_comp on id ",myid, &
       & " is different from value in restart file."
  IF (R_dump.NE.R) WRITE(*,'(a,i4,a)') "Warning: R on id ",myid, &
       & " is different from value in restart file."
  IF (Theta_dump.NE.Theta) WRITE(*,'(a,i4,a)') "Warning: Theta on id ",myid, &
       & " is different from value in restart file."
