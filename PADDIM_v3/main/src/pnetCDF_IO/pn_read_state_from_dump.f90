...
SUBROUTINE pn_read_state_from_dump(u,Temp,Chem,B,istep,t,dt)
...
#ifdef MAGNETIC
  CALL pn_check( PM_NFMPI_GET_VARA_FLOAT_ALL(ncid_dump, Bx_varid_dump,    &
              & starts, counts,B%spec(:,:,:,vec_x,ltime0))              )
#ifndef TWO_DIMENSIONAL
  CALL pn_check( PM_NFMPI_GET_VARA_FLOAT_ALL(ncid_dump, By_varid_dump,    &
              & starts, counts,B%spec(:,:,:,vec_y,ltime0))              )
#endif
  CALL pn_check( PM_NFMPI_GET_VARA_FLOAT_ALL(ncid_dump, Bz_varid_dump,    &
